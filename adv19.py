...
while True:
    register[3] = pointer
    if count > over_: print("ip=", pointer, end=" ")
    if count > over_: print(register, end=" ")
    if count > over_: print(instr[register[3]], end=" ")
    chooser(instr[register[3]],register)
    if count > over_: print(register, count)
    pointer = register[3]
    pointer += 1
    if 0 > pointer or pointer >= len(instr):
